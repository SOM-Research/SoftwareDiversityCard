...
export class MDDocumentGenerator implements Generator {
    ...
    generateSubsection(title: string, elements: any[]) : string | undefined {
        if (elements.length > 0) {
           let result = this.header(title);
            for (var elem of elements) {
                var properties = Object.getOwnPropertyNames(elem).filter(p => p !== 'id' && p !== 'participants');
                var output = this.item(elem.id, properties.map(p => this.itemize(p, elem[p])));
